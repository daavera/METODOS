...
axs[0].pcolormesh(X,Y,evolucion[-1],cmap='magma', alpha=14)
axs[1].pcolormesh(X,Y,evolucion[(int(size_t/2)-1)],cmap='magma', alpha=14)
...
def sequence(i):
    ...
    return plt_temp,
a = animation.FuncAnimation(fig,sequence,np.size(t), interval = 20, blit=False)
